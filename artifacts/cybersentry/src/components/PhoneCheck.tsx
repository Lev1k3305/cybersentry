    ...
    <div className="space-y-6">
      ...
      {checkPhone.isError && (
        ...
        >
          ...
          <AlertDescription>
            Не удалось выполнить запрос проверки номера. Пожалуйста, попробуйте
            позже.
          </AlertDescription>
        </Alert>
      )}
...
// Since the phone check results are immutable once fetched, wrapping PhoneResultCard in React.memo
// prevents redundant re-rendering of the result card, its badges, and Lucide SVG icons when the user
// continues typing/editing in the search input field.
const PhoneResultCard = React.memo(function PhoneResultCard({
  result,
}: {
  result: any;
}) {
  ...
  return (
    <Card className={`border ${borderColor} ${bgColor} overflow-hidden`}>
      <CardContent className="p-0">
        <div
          className={`p-4 border-b ${borderColor} flex items-center justify-between bg-background/40`}
        >
          ...
          <Badge
            className={`${badgeColor} font-mono tracking-widest uppercase`}
          >
            {result.risk === "safe"
              ? "Безопасно"
              : result.risk === "warning"
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div>
              <h5 className="text-xs uppercase tracking-wider text-muted-foreground font-semibold mb-1">
                Детали
              </h5>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3 bg-background/50 p-3 rounded-md border border-border/50">
              ...
              <div>
                <h5 className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">
                  Жалобы
                </h5>
                <p className="font-mono font-bold">
                  {result.calls} зафиксировано
                </p>
              </div>
            </div>
